...
var header = "/*!\n" +
	...
	" * https://github.com/haxpor/bfet/blob/master/LICENSE.md\n" +
	" */\n";
...
function buildDevTask() {
	...
	return gulp.src('dist/bfet-es6.js')
		.pipe(babel({
			presets: ['es2015']
		}))
		.pipe(streamify(concat('bfet-dev-es5.js')))
...
function buildInitialTask() {
	...
	return browserify('./src/bfet.js', { standalone: 'bfet' })
		.plugin(collapse)
		.bundle()
		.pipe(source('bfet-es6.js'))
...
function buildMinifiedTask() {
	...
	return gulp.src('dist/bfet-es6.js')
		...
		.pipe(streamify(replace('{{ version }}', package.version)))
		.pipe(streamify(concat('bfet.min.js')))
		.pipe(gulp.dest(outDir));
}
